...
module.exports = {
    ...
    execute: async (message, args) => {
        ...
        let searchActive = new MessageEmbed()
            .setColor('#000000')
            .setTitle(`Track Player`)
            .setDescription(`A search is already active in this channel.`)
            .setFooter(message.member.displayName, message.author.displayAvatarURL({ dynamic: true }))
            .setTimestamp()

        if (!search) return message.channel.send(cmdUsage);
        if (message.channel.activeCollector) return message.channel.send(searchActive);
